/**
 * @Title jQuery.drop.list.js
 * @description 下拉列表简化使用
 * @author zengrui
 * @createTime 2018/4/6 21:20
 * @modifyTime 2018/4/6 21:20
 * @return   
 **/
(function($){
    function _1(_2){
        var _3=$.data(_2,"droplist");
        var _4=_3.options;
        var _5=_3.grid;
        $(_2).addClass("droplist-f").combo($.extend({},_4,{onShowPanel:function(){
            var p=$(this).droplist("panel");
            var _6=p.outerHeight()-p.height();
            var _7=p._size("minHeight");
            var _8=p._size("maxHeight");
            var dg=$(this).droplist("grid");
    ...
    function _2e(_2f){
        ...
        $(_2f).droplist("setValues",vv);
        if(!_31.multiple){
            $(_2f).droplist("hidePanel");
        }
    };
    $.fn.droplist=function(_34,_35){
        if(typeof _34=="string"){
            var _36=$.fn.droplist.methods[_34];
            if(_36){
                return _36(this,_35);
            }else{
        ...
        return this.each(function(){
            var _37=$.data(this,"droplist");
            if(_37){
                $.extend(_37.options,_34);
            }else{
                _37=$.data(this,"droplist",{options:$.extend({},$.fn.droplist.defaults,$.fn.droplist.parseOptions(this),_34)});
            }
            _1(this);
        });
    };
    $.fn.droplist.methods={options:function(jq){
        var _38=jq.combo("options");
        return $.extend($.data(jq[0],"droplist").options,{width:_38.width,height:_38.height,originalValue:_38.originalValue,disabled:_38.disabled,readonly:_38.readonly});
    },grid:function(jq){
        return $.data(jq[0],"droplist").grid;
        ...
        return jq.each(function(){
            $(this).droplist("grid").datagrid("clearSelections");
            $(this).combo("clear");
        });
    },reset:function(jq){
        return jq.each(function(){
            var _3b=$(this).droplist("options");
            if(_3b.multiple){
                $(this).droplist("setValues",_3b.originalValue);
            }else{
                $(this).droplist("setValue",_3b.originalValue);
            }
        });
    }};
    $.fn.droplist.parseOptions=function(_3c){
        var t=$(_3c);
        return $.extend({},$.fn.combo.parseOptions(_3c),$.fn.datagrid.parseOptions(_3c),$.parser.parseOptions(_3c,["idField","textField","mode"]));
    };
    function queryDate(target,q) {
        var droplist = $.data(target,"droplist");
        var grid = droplist.grid;
        var gridOpt = grid.datagrid('options');
        //添加输入查询条件
        gridOpt.queryParams['conditions'] = [];
        gridOpt.queryParams['conditions'].push({field:gridOpt.textField,value:q,fieldAlias:gridOpt.idFieldAlias,logicOpt:'and',compareOpt:'like'});
        gridOpt.queryParams['conditions'].push({field:gridOpt.idField,value:q,fieldAlias:gridOpt.textFieldAlias,logicOpt:'or',compareOpt:'like'});
        $(grid).datagrid('reload');
    }
    var checkValue;
    $.fn.droplist.defaults=$.extend({},$.fn.combo.defaults,$.fn.datagrid.defaults,{
            height:22,
            loadMsg:null,
            idField:null,
            textField:null,
            ...
            onLoadSuccess:function (data) {
                var opt = $(this).droplist('options');
                if (data.rows.length == 1){
                    $(this).droplist('grid').datagrid('highlightRow',0);
                    $(this).droplist('grid').datagrid('selectRecord',data.rows[0][opt.idField]);
                }
            },
            loader:function (param,success,error) {
                var datagrid = $(this).datagrid('options');
                datagrid.queryParams['mainTable'] = datagrid.mainTable;
                datagrid.queryParams['joinTable'] = datagrid.joinTable;
                datagrid.queryParams['columns'] = datagrid.columns[0];
                datagrid.queryParams['rows'] = param.rows;
                datagrid.queryParams['page'] = param.page;
                if (datagrid.queryParams['conditions']== undefined){
                    datagrid.queryParams['conditions'] = [];
                }
                //添加固定查询条件
                for (var i = 0,length = datagrid.conditions.length; i < length; i++){
                    datagrid.queryParams['conditions'].push(datagrid.conditions[i]);
                }
                $.ajax({
                    type : 'POST',
                    url:ddicDroplistUrl,
                    ...
                    data : JSON.stringify(datagrid.queryParams),
            ...
            keyHandler:{
                ...
                enter:function(e){
                    _2e(this);
                    $(this).droplist("hidePanel");
                },
                query:function(q,e){
                    queryDate(this,q);
                    $(this).droplist('setValue',q);
                }}
        }
    );
    $.fn.form.defaults.fieldTypes.push('droplist');
